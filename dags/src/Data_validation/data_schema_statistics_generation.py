...
def validate_data_schema():
    ...
    # Step 5: Save training schema to output directory
    ...
    # Step 7: Save serving schema to output directory
    serving_schema_file = save_schema(schema, DATA_DIR, suffix='_serving')
    print(f"Serving schema saved to: {serving_schema_file}")
    print(f"Number of features in serving schema: {len(schema.feature)}")

    # Step 8: Generate statistics for evaluation data
    eval_stats = generate_train_stats(eval_df)

    # Step 9: Visualize statistics for training vs. evaluation data
